...
def _get_cosine_schedule_with_multiple_warmups_lambda(
    ...
    *,
    ...
    if _current_step < first_warmup_steps:
        global_lr = float(_current_step) / float(max(1, first_warmup_steps))
    else:
        if no_decay is False:
            progress = float(_current_step - first_warmup_steps) / float(max(1, num_training_steps - first_warmup_steps))
            _cosine_decay = 0.5 * (1.0 + math.cos(math.pi * progress))
            global_lr = min_lr_ratio + (1.0 - min_lr_ratio) * _cosine_decay
        else:
            global_lr = 1

    restart_step = _current_step % restart_every
    if restart_step < restart_warmup_steps:
        local_lr = float(restart_step) / float(max(1, restart_warmup_steps))
    else:
        local_lr = 1
